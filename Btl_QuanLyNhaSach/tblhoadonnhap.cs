...
namespace Btl_QuanLyNhaSach
{
    public partial class tblhoadonnhap : Form
    {
        ...
        private void GetValuesTextBox()
        {
            ...
            hoadonnhap = new HoaDonNhap(smaHDNhap, dngayNhap);
        }
        ...
        private void btnCapNhatHDNhap_Click(object sender, EventArgs e)
        {
            ...
            {
                GetValuesTextBox();
                string query = "UPDATE tblHoaDonNhap SET dNgayNhap = '" + hoadonnhap.DNgayNhap + "' ";
                query += "WHERE sMaHDNhap = '" + hoadonnhap.SMaHDNhap + "'";
                try
                {
        ...
        private void btnThemHDNhap_Click(object sender, EventArgs e)
        {
            ...
            {
                GetValuesTextBox();
                string query = "INSERT INTO tblHoaDonNhap (sMaHDNhap, dNgayNhap) values ('" + hoadonnhap.SMaHDNhap + "', '" + hoadonnhap.DNgayNhap + "' ) ";
                try
                {
        ...
        private void dataGridView_HDNhap_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridView_HDNhap.Rows.Count > 1)
            {
                ...
                dNgayNhap.Text = dataGridView_HDNhap.SelectedRows[0].Cells[1].Value.ToString();
            }
        }
        ...
        private void tblhoadonnhap_Load(object sender, EventArgs e)
        {
            try
            {
                dataGridView_HDNhap.DataSource = modify.Table("SELECT sMaHDNhap AS 'Mã HĐ Nhập', dNgayNhap AS 'Ngày Nhập' FROM tblHoaDonNhap");
            }
            catch (Exception ex)
            {
        ...
        private void sTimKiemMaHDNhap_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                string query = "SELECT sMaHDNhap AS 'Mã HĐ Nhập', dNgayNhap AS 'Ngày Nhập' FROM tblHoaDonNhap WHERE sMaHDNhap LIKE N'%" + name + "%'";
